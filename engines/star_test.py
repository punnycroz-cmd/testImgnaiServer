...
class StarTestManager:
    def __init__(self, cookie_dir: str, output_dir: str, vault: R2Vault, db=None, cancelled_jobs=None):
        ...
        self.logger = logging.getLogger("aether.star_test")

    async def generate(self, req, request_id=None):
        if request_id and request_id in self.cancelled_jobs:
            self.logger.info("star test generate cancelled before start: %s", request_id)
            return None

        script_path = os.path.join(os.getcwd(), "star_test_api.py")
        cmd = [
        ...
        async with self._lock:
            ...
            while True:
                ...
                if line:
                    ...
                    if len(tail) > 8:
                        tail.pop(0)
                    if request_id and request_id in self.cancelled_jobs:
                        self.logger.warning("star test job cancelled during execution, terminating: %s", request_id)
            ...
            if rc != 0:
                ...
                raise RuntimeError(f"Star Test failed (code {rc}): {reason}")
            if not last_json_line: raise RuntimeError("Engine output missing")
            
            data = json.loads(last_json_line)
            session_uuid = data.get("session_uuid", "session")
            task_uuids = data.get("task_uuids", [])
            image_urls = data.get("image_urls", [])

        batch_prefix = self.vault.build_batch_prefix_with_name("star_test", session_uuid, ts=datetime.now())
        ...
        return {"image_urls": vaulted_urls, "client_id": getattr(req, "client_id", None), "model": req.model, "prompt": req.prompt}
